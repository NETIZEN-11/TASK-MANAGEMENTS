...
export default function SignupForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    // FIX: enforce the same password rules the backend Joi schema does
    // (min 8, max 128, must contain at least one letter AND one digit).
    // Without this the request reaches the server and is rejected with 400,
    // and the user only sees a generic "Signup failed".
    if (!validatePassword(password)) {
      setError('Password must be 8–128 characters and include a letter and a number');
      return;
    }
    setSubmitting(true);
    try {
      ...
      // FIX: prefer the field-level detail from the server when available
      // (e.g. "Password must contain at least one digit").
      const detail = err?.details?.[0]?.message;
      setError(detail || err?.message || 'Signup failed');
    } finally {
      // FIX BUG-H09: always clear submitting.
      setSubmitting(false);
    }
  };
  ...
  return (
    ...
    >
      <div className="mb-7">
        ...
        <p className="mt-2 text-sm text-white/70">Start managing your tasks in seconds.</p>
      </div>

      <div className="mb-5">
        <label className="mb-2 block text-xs font-medium text-white/80" htmlFor="signup-name">
          Full name
        </label>
      ...
      <div className="mb-5">
        <label className="mb-2 block text-xs font-medium text-white/80" htmlFor="signup-email">
          Email address
        </label>
        <div className="relative group">
          <span className={iconClass}>
      ...
      <div className="mb-7">
        <label className="mb-2 block text-xs font-medium text-white/80" htmlFor="signup-password">
          Password
        </label>
        <div className="relative group">
          <span className={iconClass}>
      ...
      <SubmitButton
        ...
        hoverShadow="hover:shadow-[0_10px_30px_rgba(16,185,129,0.45)]"
      />
      ...
      <p className="mt-6 text-center text-sm text-white/60">
        ...
        <Link
          to="/login"
          className="font-semibold text-white underline-offset-4 transition-all hover:underline hover:text-emerald-200"
        >
          Log in
        </Link>
      </p>
